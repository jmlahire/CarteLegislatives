...
function selectAll$1(selector) {
  return typeof selector === "string"
      ...
      : new Selection$3([array$2(selector)], root$2);
}
...
function bisector$1(f) {
  ...
  if (f.length !== 2) {
    ...
    compare1 = f === ascending$2 || f === descending ? f : zero$2;
    compare2 = f;
    delta = f;
  }
...
const bisectCenter = bisector$1(number$6).center;
...
function bin() {
  ...
  function histogram(data) {
    ...
    if (!Array.isArray(tz)) {
      const max = x1, tn = +tz;
      if (domain === extent$1) [x0, x1] = nice$2(x0, x1, tn);
      tz = ticks$1(x0, x1, tn);
  ...
  histogram.thresholds = function(_) {
    return arguments.length ? (threshold = typeof _ === "function" ? _ : Array.isArray(_) ? constant$7(slice$1.call(_)) : constant$7(_), histogram) : threshold;
  };
...
function genericArray$1(a, b) {
  ...
  for (i = 0; i < na; ++i) x[i] = interpolate$4(a[i], b[i]);
  for (; i < nb; ++i) c[i] = b[i];
...
function parseCss$1(value) {
  ...
  return m.isIdentity ? identity$b : decompose$1(m.a, m.b, m.c, m.d, m.e, m.f);
}
...
function parseSvg$1(value) {
  ...
  if (!(value = svgNode$1.transform.baseVal.consolidate())) return identity$b;
  value = value.matrix;
...
function piecewise(interpolate, values) {
  if (values === undefined) values = interpolate, interpolate = interpolate$4;
...
function transformer$4() {
  ...
  function rescale() {
    ...
    if (clamp !== identity$a) clamp = clamper$1(domain[0], domain[n - 1]);
  ...
  scale.clamp = function(_) {
    return arguments.length ? (clamp = _ ? true : identity$a, rescale()) : clamp !== identity$a;
  };
...
function continuous$1() {
  return transformer$4()(identity$a, identity$a);
}
...
function formatLocale$3(locale) {
  var group = locale.grouping === undefined || locale.thousands === undefined ? identity$9 : formatGroup$1(map$1.call(locale.grouping, Number), locale.thousands + ""),
      currencyPrefix = locale.currency === undefined ? "" : locale.currency[0] + "",
      currencySuffix = locale.currency === undefined ? "" : locale.currency[1] + "",
      decimal = locale.decimal === undefined ? "." : locale.decimal + "",
      numerals = locale.numerals === undefined ? identity$9 : formatNumerals$1(map$1.call(locale.numerals, String)),
...
function powish(transform) {
  var scale = transform(identity$a, identity$a),
      exponent = 1;

  function rescale() {
    return exponent === 1 ? transform(identity$a, identity$a)
...
var millisecond$2 = newInterval$1(function() {
  // noop
}, function(date, step) {
...
function ticker$1(year, month, week, day, hour, minute) {
  ...
  const tickIntervals = [
    [utcSecond$1,  1,      durationSecond$1],
...
function transformer$3() {
  ...
  scale.range = range(interpolate$4);
...
function sequential() {
  var scale = linearish$1(transformer$3()(identity$a));
...
function diverging() {
  var scale = linearish$1(transformer$2()(identity$a));
...
var d3Timer = /*#__PURE__*/Object.freeze({
  __proto__: null,
  now: now$1,
  timer: timer$1,
  ...
  if (schedules) {
    ...
    for (i in schedules) {
      if ((schedule = schedules[i]).state > SCHEDULED$1 && schedule.name === name) {
        return new Transition$1([[node]], root$1, name, +i);
      }
    }
  }
...
const d3$b = Object.assign({}, d3Selection, d3Dispatch, d3Transition);
...
class Component extends Queue {
  ...
  constructor(id) {
    ...
    this._style = {};
    this._dispatch = d3$b.dispatch('user');
    this._container = undefined;
    this._transition = d3$b.transition().duration(750);
    Registry.add(this);
  }
  /**
  ...
  appendTo(parent) {
    ...
    if (!this.outerContainer) console.warn(`Aucun contenu présent dans le composant ${this.id}`); //Determination du type de parent
    else {
      this._parent = {
        component: null,
        container: undefined
      ...
      else if (parent instanceof d3$b.selection) {
        this._parent.container = parent;
      } //Selection d3 (passé par son id)
      else if (typeof parent === 'string') {
        this._parent.container = d3$b.select(`#${parent.replace('#', '')}`);
      } else this._parent.container = d3$b.select('body'); //Insertion dans le DOM
  ...
  opacity(o, options = {}) {
    ...
    this.outerTransition = this.outerContainer.transition().style('opacity', o).on('end', () => this._style.opacity = o);
    return this;
  }
...
class SvgSize {
  constructor({
    ...
    this.width = width;
    this.height = height;
    this.margins = isNaN(margins) ? margins : {
      top: margins,
      right: margins,
      bottom: margins,
      left: margins
    ...
    const handler = {
      get(target, prop) {
        if (typeof target[prop] === 'object' && target[prop] !== null) return new Proxy(target[prop], handler);else return target[prop];
      },
...
class Svg extends Component {
   ...
   * @param size
   */
  constructor(id, width = 2000, height = 1200, margins = {
    top: 50,
    right: 50,
    bottom: 50,
    left: 50
    ...
    super(id);
    this.size = new SvgSize({
      width,
      height,
      margins
    });
    this._outerContainer = d3$a.create('svg:svg').attr('id', this.id).attr(`preserveAspectRatio`, 'xMaxYMin meet').attr('viewBox', `0 0 ${this.size.width} ${this.size.height}`).attr('width', `100%`);
    this._innerContainer = this._outerContainer.append('svg:g').attr('class', '_svgContent').attr('transform', `translate(${this.size.margins.left} ${this.size.margins.top})`);
  }
...
function drag() {
  ...
  function beforestart(that, container, event, d, identifier, touch) {
    var dispatch = listeners.copy(),
        p = pointer$1(touch || event, container), dx, dy,
        s;
    ...
    return function gesture(type, event, touch) {
      ...
      switch (type) {
        ...
        case "drag": p = pointer$1(touch || event, container), n = active; break;
      }
      dispatch.call(
        type,
        that,
...
function zoom() {
  ...
  function mousedowned(event, ...args) {
    ...
    function mousemoved(event) {
      ...
      g.event(event)
       .zoom("mouse", constrain(translate(g.that.__zoom, g.mouse[0] = pointer$1(event, currentTarget), g.mouse[1]), g.extent, translateExtent));
    }
...
const d3$9 = Object.assign({}, d3Selection, d3Geo, d3Zoom, d3Transition, d3Ease, d3Dispatch);
...
class MapLayer extends Component {
  ...
  constructor(id, className = '', parent) {
    super(id);
    this._dispatch = d3$9.dispatch('click');
    this._container = d3$9.create('svg:g').attr('id', this.id).style('opacity', 0).classed(className, className);
  ...
  get projection() {
    return this.parent.projection || d3$9.geoMercator();
  }
  /**
  ...
  get path() {
    return this.parent.path || d3$9.geoPath();
  }
...
const d3$8 = Object.assign({}, d3Selection, d3Fetch, d3Array, d3Geo, d3Zoom, d3Transition, d3Ease, d3Dispatch);
...
class MapPath extends MapLayer {
  ...
  constructor(id, parent, options = {}) {
    ...
    this._options = { ...MapPath.defaultOptions,
      ...options
    };
  }
  /**
  ...
  load(arg) {
    ...
    this.parent.enqueue(() => new Promise(resolve => {
      d3$8.json(source).then(topology => {
  ...
  render() {
    ...
    this.parent.enqueue(() => new Promise(resolve => {
      ...
      this._container.selectAll("path").data(this.geodata, d => d.properties[this._options.primary]).join(enter => enter.append('path').attr('class', d => `path _${d.properties[this._options.primary]}`).classed('clickable', this._options.clickable).attr('d', this.path).on('click', (e, d) => {
        // console.warn(e, e.detail);
        e.stopPropagation();

        if (this._options.clickable && !e.target.classList.contains('hidden')) {
          // && e.detail===1
          this._dispatch.call('click', this, {
            event: e,
            values: d.properties,
            id: d.properties[this._options.primary]
          });

          this.highlight(d.properties[this._options.primary]);
        }
      }), update => update.call(update => update.transition().duration(0).attr('d', this.path)), exit => exit.remove());
  ...
  join(dataCollection, dataKey, geoKey) {
    ...
    this.parent.enqueue(() => new Promise(resolve => {
      dataCollection.ready.then(data => {
        ...
        this._container.selectAll("path").each(d => {
  ...
  statistics(keys, types = ['domain', 'mean']) {
    ...
    const calcFunction = {
      ...
      sum: d3$8.sum,
      count: d3$8.count,
      mean: v => calcRound(d3$8.mean(v)),
      median: v => calcRound(d3$8.median(v)),
      deviation: v => calcRound(d3$8.deviation(v, .1))
    };
    this.parent.enqueue(() => new Promise(resolve => {
  ...
  fill(stylingFunction) {
    this.parent.enqueue(() => new Promise(resolve => {
      this._container.selectAll("path").each((d, i, n) => {
        const style = stylingFunction(d.properties),
              transition = d3$8.select(n[i]).transition().duration(this.parent._options.duration / 2); //Couleur simple

        if (typeof style.fill === 'string') {
          transition.style('fill', style.fill);
        } //Pattern
        else if (typeof style.fill === 'object') {
          //&& style.fill.constructor.name==='Selection$4'
          ...
          transition.style('fill', color).on('end', function () {
            d3$8.select(this).style('fill', `url(#${id})`);
          });
        } //Blank
        else {
          transition.style('fill', '#ccc');
          d3$8.select(n[i]).classed('clickable', false);
        } //Styles additionnels
  ...
  highlight(id) {
    if (id) this._container.selectAll('path.clickable').classed('highlight', d => d.properties[this._options.primary] === id).filter(d => d.properties[this._options.primary] === id).raise();else this._container.selectAll('path.clickable').classed('highlight', false);
    return this;
  }

  zoomTo(key, value) {
    let selection = this.innerContainer.selectAll('path.clickable').classed('hidden', d => d.properties[key] !== value).filter(d => d.properties[key] === value);
  ...
  zoomOut() {
    this.innerContainer.selectAll('path.clickable').classed('hidden', false);
    this.parent.zoomOut();
...
_defineProperty(MapPath, "_type", '_mPth');

_defineProperty(MapPath, "defaultOptions", {
  primary: 'ID',
  autofit: "auto",
  clickable: true
});

Object.assign({}, d3Selection, d3Fetch, d3Array, d3Geo, d3Zoom, d3Transition, d3Ease, d3Dispatch);
...
_defineProperty(MapCustom, "_type", '_mCust');

const d3$7 = Object.assign({}, d3Selection, d3Geo, d3Zoom, d3Transition, d3Ease, d3Dispatch);
...
class MapComposition extends Svg {
  ...
  constructor(id, size = {}, options = {}) {
    ...
    this._options = { ...MapComposition.defaultOptions,
      ...options
    }; // this.outerContainer.classed(MapComposition._type,true);

    this._layers = new Map();
    this._defs = this.outerContainer.append('defs').lower();
    this._dispatch = d3$7.dispatch('zoom'); //this._zoomEvent={e:null,level:1,source:null};

    this._zoomSource = this.outerContainer;
    this._zoom = d3$7.zoom().filter(e => !e.button && e.type != "dblclick").scaleExtent([1, 15]).translateExtent([[0, 0], [this.size.width, this.size.height]]).on('zoom', e => this._handleZoom.call(this, e));

    this._zoomSource.call(this._zoom); // this.freezoom(this._options.freezoom);


    this.projection = this._options.projection;
    this.path = d3$7.geoPath();
  }
  /**
  ...
  _addLayer(id, options) {
    let newLayer;
    if (options.type === 'path') newLayer = new MapPath(id, this, options);else if (options.type === 'custom') newLayer = new MapCustom(id, this, options);
  ...
  _handleZoom(e) {
    ...
    if (e.sourceEvent === null) {
      ...
      this._dispatch.call('zoom', this, {
        level: e.transform.k,
        source: e.sourceEvent
      });
    } //Zoom manuel
    else if (e.sourceEvent instanceof MouseEvent) ; // console.log(this._zoomEvent);
  ...
  zoomTo(selection, options = {}) {
    ...
    this.enqueue(() => new Promise((resolve, reject) => {
      selection = selection.nodes(); //Calcul du zoom
      ...
      const finalTransform = d3$7.zoomIdentity.translate(offset.x, offset.y).scale(scale);

      this._zoomSource.transition().delay(this._options.delay).duration(this._options.duration).call(this._zoom.transform, finalTransform).on('end', () => {
  ...
  zoomOut() {
    this.enqueue(() => new Promise((resolve, reject) => {
      let finalTransform = d3$7.zoomIdentity.translate(0, 0).scale(1);

      this._zoomSource.transition().delay(this._options.delay).duration(this._options.duration).call(this._zoom.transform, finalTransform).on('end', () => {
        this._zoom.scaleExtent([1, finalTransform.k * 4]);

        this.outerContainer.call(this._zoom, finalTransform); //  this.container.selectAll('g#departements path.area').style('pointer-events','visible'); //Hack degueu
  ...
  fadeOutLayers(selector) {
    //  this.enqueue( () => new Promise((resolve, reject) => {
    this.container.selectAll(`g${selector}`).transition().duration(this._options.duration / 2).style('opacity', 0).on('end', (d, i, n) => {
      d3$7.select(n[i]).style('display', 'none'); //   resolve(this);
    }); //    }));
  ...
  fadeInLayers(selector) {
    // this.enqueue( () => new Promise((resolve, reject) => {
    this.container.selectAll(`g${selector}`).style('display', 'auto').transition().duration(this._options.duration / 2).style('opacity', 1); //        .on('end', ()=>resolve(this));
    //  }));
...
_defineProperty(MapComposition, "_type", '_mComp');

_defineProperty(MapComposition, "defaultOptions", {
  duration: 500,
  delay: 0,
  projection: d3$7.geoMercator(),
  freezoom: false
});

const d3$6 = Object.assign({}, d3Selection, d3Dispatch);
...
class MapLegend extends Component {
  ...
  constructor(id, title = 'Légende') {
    super(id);
    this._outerContainer = d3$6.create('div').classed(MapLegend._type, true);
  ...
  categories(data, meta = {}) {
    ...
    this._innerContainer.selectAll("figure").data(data, d => d.id).join(enter => enter.append("figure").each(function (d) {
      const elt = d3$6.select(this);
      elt.append('span').attr('class', 'color').style('background', d[meta.color]) //.attr('title', d=>d[meta.name])
...
_defineProperty(MapLegend, "_type", '_mLeg');

let _Symbol$iterator;
const d3$5 = Object.assign({}, d3Array, d3Fetch, d3Dsv);
...
function applyDtype(dataset, dtype) {
  ...
  dataset.map((d, i) => {
    dtype.forEach((type, key) => {
      try {
        if (type === "int") d[key] = parseInt(d[key]);else if (type === "float") d[key] = parseFloat(d[key]);else if (type === "boolean") d[key] = !!d[key];
      } catch (error) {
        console.warn(error);
        d[key] = null;
      }
...
class DataCollection {
  ...
  load(file, options = {}) {
    this.ready = new Promise((resolve, reject) => {
      ...
      d3$5.text(`${file}`).then(dataset => {
        dataset = d3$5.dsvFormat(options.delimiter).parse(dataset);
        if (options.dtype) dataset = applyDtype(dataset, options.dtype);else dataset = dataset.map(d3$5.autoType);
  ...
  _export(primary, type = 'Dictionary') {
    ...
    if (type.toLowerCase() === 'map') {
      ...
      this.toGroups(primary).forEach(d => this.datasetMap.set(d[0], d[1]));
      return this.datasetMap;
    } else if (type.toLowerCase() === 'dictionary') {
      this.datasetDict = new Object();
      this.toGroups(primary).forEach(d => this.datasetDict[d[0]] = d[1]);
  ...
  toGroups(keys, output = 'array') {
    const groupFn = output === 'array' ? d3$5.groups : d3$5.group;
    if (typeof keys === 'string') keys = [keys];
  ...
  extent(key) {
    if (typeof key == 'string') {
      return d3$5.extent(this.col(key));
  ...
  uniques(keys) {
    ...
    this.dataset.forEach(d => {
      let reduced = {};
      keys.forEach(k => reduced[k] = d[k]); //  console.log(reduced);
  ...
  sort(key, type = 'descending', inPlace = true) {
    let dataset = this.dataset.sort((a, b) => d3$5[type](a[key], b[key]));
...
_defineProperty(DataCollection, "type", 'DataCollection');

const d3$4 = Object.assign({}, d3Selection, d3Dispatch);
...
class DropdownList {
  constructor(level, metadata, autoselect = true) {
    ...
    this._outerContainer = d3$4.create('li').attr('class', `ddList${level}`);
    this._innerContainer = this._outerContainer.append('select');
    this._dispatch = d3$4.dispatch('change');
  }
  ...
  update(data = null, selected = null) {
    ...
    const getValue = d => data instanceof Map ? d[0] : d[this._metadata.value],
          getText = d => data instanceof Map ? traverseText(d[1]) : d[this._metadata.text]; //Données vides -> on n'affiche rien
    ...
    if (data === null) {
      ...
      if (!this._autoselected) {
        ...
        this._dispatch.call('change', this, {
          level: this.level,
          value: value
        });
      ...
      this._innerContainer.selectAll('option').data(data).enter().append('option').property('selected', d => getValue(d) === selected).attr('value', getValue).text(getText);
  ...
  addPlaceHolder(text) {
    this._innerContainer.append('option').property('disabled', true).property('selected', true).property('hidden', true).text(text).lower();
  ...
  addOption(value, text, firstPosition = true) {
    if (this.findValue(value) === null) {
      let option = this._innerContainer.append('option').attr('value', value).text(text);
  ...
  findValue(value) {
    ...
    return found >= 0 ? d3$4.select(options[found]) : null;
  }

  select(value) {
    this._innerContainer.selectAll('option').each(d => console.log(d)); //TODO
...
class NestedSelection extends Component {
  constructor(id) {
    super(id);
    this._dispatch = d3$4.dispatch('select');
    this._outerContainer = d3$4.create('div').attr('id', this.id).classed(NestedSelection._type, true);
  ...
  data(data, metadata, options = {}, selection) {
    ...
    for (let i = 0; i < this._metadata.length; i++) {
      this._sections[i] = new DropdownList(i, this._metadata[i]).appendTo(this).on('change', e => {
        //Modification des valeurs sélectionnées
        this._selected[e.level] = e.value === 'null' ? null : e.value;

        for (let i = e.level + 1; i < this._metadata.length; i++) this._selected[i] = null; //MAJ des selects


        this.update(); //Renvoi du dispatch
        ...
        this._dispatch.call('select', this, {
          level: e.level,
          value: value
        });
      });
  ...
  select(selection, dispatch = true) {
    ...
    if (needle) {
      ...
      this.update();
      if (dispatch) this._dispatch.call('select', this, {
        level: level,
        value: value
      });
    }
...
_defineProperty(NestedSelection, "_type", '_nNestSel');

const d3$3 = Object.assign({}, d3Selection, d3Dispatch);
...
class FigureFactory {
  ...
  render(type, colors) {
    ...
    return d3$3.create('svg:svg').attr('preserveAspectRatio', 'xMaxYMin meet').attr('viewBox', `0 0 ${size.width} ${size.height}`).html(code);
  }
...
class Synthese extends Component {
  ...
  constructor(id) {
    super(id);
    this._container = d3$3.create('div').attr('id', id);
    this._dispatch = d3$3.dispatch('select');
  ...
  _renderElus(data) {
    if (!data.size) this._sectionElus.style('display', 'none');else this._sectionElus.select('ul').selectAll('li').data(data).enter().append('li').each((d, i, n) => {
      let circo = d[0],
          color = d[1].nuanceCol,
          figure = d[1].civ === 'M' ? 'man' : 'woman';
      let svg = new FigureFactory().render(figure, color);
      d3$3.select(n[i]).attr('class', `_${circo}`).append(() => svg.node()).on('click', () => this._dispatch.call('select', this, {
        circo: circo
      }));
    });
  }
  ...
  _renderTriangulaires(data) {
    if (!data.size) this._sectionTriang.style('display', 'none');else this._sectionTriang.select('ul').selectAll('li').data(data).enter().append('li').each((d, i, n) => {
      let circo = d[0],
...
_defineProperty(Synthese, "_type", '_cSynth');

const FormatFloat = (value, digits = 2, errorValue = '-') => {
  let toNumber = parseFloat(value);
  return isNaN(toNumber) ? errorValue : value.toLocaleString('fr-FR', {
    minimumFractionDigits: digits,
    maximumFractionDigits: digits
  });
};

const FormatInt = (value, errorValue = '-') => {
  let toNumber = parseInt(value);
  return isNaN(toNumber) ? errorValue : toNumber.toLocaleString('fr-FR');
};

const FormatPercent = (value, digits = 2, errorValue = '-') => {
  return isNaN(parseFloat(value)) ? errorValue : `${FormatFloat(value, digits)}%`;
};

const d3$2 = Object.assign({}, d3Selection, d3Dispatch, d3Array, d3Fetch, d3Dsv);
...
class Results extends Component {
  ...
  constructor() {
    super('Resultats');
    this._container = d3$2.create('div');
  }

  data(listeCircos, nuancesPol) {
    this._circos = listeCircos;
    this._nuances = d3$2.group(nuancesPol.dataset, d => d.code);
    return this;
  }

  update(circo) {
    this.id = circo;
    this._metadata = this._circos.find('CIRCO_ID', circo);

    this._container.attr('id', `res${circo}`);
  ...
  _title() {
    let title = [],
        circo = this._metadata['CIRCO_NOM'] === 1 ? '1<sup>ère</sup>' : `${this._metadata['CIRCO_NOM']}<sup>e</sup>`;
    if (this._metadata['DEP_NOM']) title.push(this._metadata['DEP_NOM'].toUpperCase());
    title.push(circo + ' circonscription');
    this._header = this._container.append('header');

    this._header.append('h3').text(this._metadata['REG_NOM']);

    this._header.append('h4').html(title.join(' / '));
  ...
  _load() {
    this.enqueue(() => new Promise(resolve => {
      let fileName = `assets/data/circonscriptions/${this.id.replace('-', '')}.csv`;

      try {
        d3$2.csv(fileName).then(dataset => {
          this._data = dataset.map(d3$2.autoType).sort((a, b) => b.voix - a.voix);
          resolve(this._data);
        });
      } catch (error) {
        console.warn(error);
      }
      /*   new DataCollection(`res${this.id}`)
             .load(`assets/data/circonscriptions/${this.id.replace('-','')}.csv`)
             .ready.then( (d)=>  {this._data=d.dataset.sort((a,b)=> b.voix-a.voix ); resolve(d.dataset);});*/

    }));
  }

  render(delay = 0) {
    delay = !this._style.opacity ? delay : 0;
    setTimeout(() => {
      this.enqueue(() => new Promise(resolve => {
        this._title();

        let inscrits = this._data[0].inscrits;
            this._data[0].votants;
            this._data[0].exprimes;
 //this._container.append('p').text(`Participation : ${FormatPercent(participation)}`);
        ...
        for (const [key, value] of figs) {
          let tr = thead.append('tr'),
              td0 = value.label || key.charAt(0).toUpperCase() + key.slice(1),
              td1 = FormatInt(this._data[0][key]),
              td2 = FormatPercent(100 * this._data[0][key] / inscrits);
          tr.append('td').text(td0).style('padding-left', '1.2rem');
          tr.append('td').text(td1).classed('right', true);
          tr.append('td').text(td2).classed('bold', value.bold).classed('right', true);
          tr.append('td').text('').attr('colspan', 2).classed(value.className, value.className);
        }

        this._container.select('td.participation').html('&nbsp;&nbsp;&nbsp;PARTICIPATION').classed('bold', true);

        table.append('tr').append('td').attr('colspan', '5').classed('separator', true);
        table.append('tr').html('<th rowspan="2"></th><th rowspan="2" style="vertical-align: bottom;">voix</th><th colspan="2" style="text-align: center;">&nbsp;&nbsp;Pourcentage</th><th rowspan="2"></th>');
        table.append('tr').html('<th>inscrits</th><th>exprimés</th>');
        let ptd4;

        this._data.forEach((d, i) => {
          let tr = table.append('tr'),
              color = this._nuances.has(d.nuance) ? this._nuances.get(d.nuance)[0].couleur : '#ccc',
              td0 = `<span style="background-color:${color};"></span>${d.nom} ${d.prenom} (${d.nuance})`,
              td1 = FormatInt(d.voix),
              td2 = FormatPercent(d.pct_inscrits),
              td3 = FormatPercent(d.pct_exprimes),
              td4 = d.elu === 'O' ? 'élu(e)' : d.elu === 'B' ? 'ballotage' : '';
          if (td4 !== '') tr.classed('bold', true);
          tr.append('td').html(td0).filter(() => td4 === '' && ptd4 !== '' || i === 0).style('padding-top', '2vh');
          tr.append('td').text(td1).classed('right', true);
          tr.append('td').text(td2).classed('right', true);
          tr.append('td').text(td3).classed('right', true);
          tr.append('td').text(td4).classed('right', true);
          ptd4 = td4;
        });
  ...
  show() {
    d3$2.select('div._Panel').style('display', 'none');

    this._parent.container.transition().duration(500).style('background', 'rgba(255,255,255,.9)');

    this.fadeIn();

    this._container.style('pointer-events', 'auto').on('click', () => this.hide());
  }

  hide() {
    d3$2.select('div._Panel').style('display', 'block');

    this._parent.container.transition().duration(500).style('background', 'none');
...
_defineProperty(Results, "_type", '_cResult');

const d3$1 = Object.assign({}, d3Selection, d3Timer, d3Dispatch, d3Drag);
...
class Panel extends Component {
  ...
  constructor(id, options) {
    super(id);
    this._options = { ...Panel.defaultOptions,
      ...options
    };
    this._position = this._options.initialPosition;
    this._dispatch = d3$1.dispatch('position');
    this._timer = new d3$1.timeout(() => {}, 0);
    this._outerContainer = d3$1.create('div').classed(Panel._type, true).classed('right', true);
    ...
    this._outerContainer.on('click', e => {
      if (this._position === 'unfolded') {
        this.fold();

        this._outerContainer.call(d3$1.drag().on('start', null).on('end', null));
      } else {
        this.unfold();

        this._outerContainer.call(d3$1.drag().on("start", e => {
          if (this._position === 'folded') this.unfold();else {
            this._pos.x = e.x;
            this._pos.y = e.y;
          }
        }).on("end", e => {
  ...
  resize() {
    ...
    if (this._options.minWidth) this.size.width = Math.max(this.size.width, Panel._convertLengthToPx(this._options.minWidth)); //Dimensionnement du panel
    ...
    this._handle.append('span').style('line-height', `${this.size.height}px`).style('font-size', `${this.size.handle * .7}px`).text('◀').classed('txt', () => this._options.handleText).filter(() => this._options.handleText).text(this._options.handleText).style('transform', `translateX(-${this.size.handle * 1.05}px) rotate(-90deg)`); //Dimensionnement et positionnement contenu


    this.size.content = this.size.width - this.size.handle;
    this.innerContainer.style('left', `${this.size.handle}px`).style('width', `${this.size.width - this.size.handle}px`).style('height', `${this.size.height}px`); //Position initiale

    if (this._options.initialPosition === 'unfolded') this.outerContainer.classed('unfolded', true).style('right', '0px');else {
      this.outerContainer.classed('folded', true).style('right', `-${this.size.content}px`);
    }
    return this;
  }
  /**
  ...
  fold(options = {}) {
    ...
    this.enqueue(() => new Promise((resolve, reject) => {
      this.outerContainer.transition().duration(options.duration).delay(options.delay).style('opacity', .5).style('right', `-${this.size.content}px`).on('end', () => {
        this._dispatch.call('position', this, {
          action: 'folded'
        });
  ...
  unfold(options = {}) {
    ...
    this.enqueue(() => new Promise((resolve, reject) => {
      this.outerContainer.transition().duration(options.duration).delay(options.delay).style('opacity', 1).style('right', '0px').on('end', () => {
        this._dispatch.call('position', this, {
          action: 'unfolded'
        });
...
_defineProperty(Panel, "_type", '_Panel');
...
const nuancesPol = new DataCollection("nuances").load("./assets/data/nuances.csv", {
  primary: "id"
});
const listeCircos = new DataCollection('listeCircos').load("./assets/data/listecircos.csv", {
  delimiter: ';',
  primary: 'CIRCO_ID',
  dtype: {}
}); //REG_ID:'int'

const resultats = new DataCollection("resultats").load("./assets/data/sample.csv", {
  primary: "id",
  dtype: {
    circo: "int",
    pano: "int",
  ...
  mapper: d => {
    d.idcirco = `${d.id.substring(0, 3)}-${d.id.substring(3, 5)}`;
    return d;
  }
});
/**********************************************************************
...
function getUrlParam() {
  let urlParams = [getUrlParam$1('reg'), getUrlParam$1('dep'), getUrlParam$1('circo')];
...
resultats.statistics = function (nuancesPol) {
  ...
  let nuancesList = new Set(),
      //Nuances arrivées en tête dans au mojns une circo
  nuancesCompteur = {
    tete: new Array(nuancesPol.map.size).fill(0),
    qualif: new Array(nuancesPol.map.size).fill(0)
  },
      syntheseResultats = {
    elus: new Map(),
    duels: new Map(),
  ...
  nuancesList = Array.from(nuancesList).map(d => Object.assign(d, {
    tete: nuancesCompteur.tete[d.id],
    qualif: nuancesCompteur.qualif[d.id]
  })).sort((a, b) => d3.descending(a.tete, b.tete));
  return {
    nuances: {
      compteur: nuancesCompteur,
      list: nuancesList
    },
...
function mapFillingFunction(properties) {
  ...
  try {
    ...
    if (first) {
      const color = nuancesPol.map.get(first.nuanceId)[0].couleur; //Election au premier tour
...
function selectCirco(selection, origin) {
  ...
  if (origin instanceof PointerEvent) {
    ...
    mySelector.select(selection, false);
    setTimeout(() => {
      myMap.layer('circos').highlight(selection[2]); //Metropole

      if (selection[2].substring(0, 3) !== '099' && selection[2].charAt(0) !== '9') {
        myMap.layer('circos').zoomTo('DEP3', mySelector.selection[1]); //showResults(selection[2],1000);
      } //Outremer et etranger
      else {
        myMap.layer('circos').zoomOut(); //showResults(selection[2],1000);
      }
    }, 500);
    ...
    mySelector.select(selection, false);
    if (mySelector.selection[1] !== null) myMap.layer('circos').zoomTo('DEP3', mySelector.selection[1]);else if (mySelector.selection[0] !== null) myMap.layer('circos').zoomTo('REG', mySelector.selection[0]);
  }
}

function showResults(circo, delay = 0) {
  contentBox.update(circo).render(delay);
}
/**********************************************************************
 ***************************** CARTES *********************************
 **********************************************************************/
...
function renderDomLabels() {
  ...
  this._container.append('text').attr('x', doms.x0 + 15).attr('y', doms.y0 - fontSize * 2).text("Outremer").style('font-size', `${fontSize * 1.5}px`).style('text-anchor', 'start');

  [971, 972, 973, 974, 975, 976, 977, 986, 987, 988].forEach((d, i) => {
    this._container.append('text').attr('x', doms.x0).attr('y', doms.y0 + i * doms.gap).text(d).style('font-size', `${fontSize}px`).style('text-anchor', 'end');
  });
  const etranger = {
    x0: 510,
    y0: 2050,
  ...
  const label = this._container.append('text').attr('x', doms.x0 + 20).attr('y', etranger.y0 + fontSize).style('font-size', `${fontSize * 1.5}px`).style('text-anchor', 'start');
  ...
  for (let i = 0; i < 11; i++) {
    this._container.append('text').attr('x', etranger.x0 + i * etranger.gap).attr('y', etranger.y0).text(i + 1).style('font-size', `${fontSize}px`).style('text-anchor', 'middle');
  }
}

const myMap = new MapComposition('CarteFrance', {
  width: 2000,
  height: 2200,
  margins: 10
...
myMap.layer('circos', {
  type: 'path',
  source: 'assets/geodata/circonscriptions-simplifie.topojson',
  primary: 'ID',
  autofit: true,
  clickable: true
});
myMap.layer('depts', {
  type: 'path',
  source: 'assets/geodata/departements-simplifie.topojson',
  primary: 'DEP',
  autofit: "auto",
  clickable: false
});
...
Promise.all([nuancesPol.ready, resultats.ready, listeCircos.ready]).then(() => {
  //Création des cartes
  myMap.layer('circos').load().fit().render().on('click', d => selectCirco([null, null, d.id], d.event)).join(resultats, "idcirco").fill(mapFillingFunction);
  myMap.layer('depts').load().render();
  myMap.layer('labels').render(); //Ajout de la légende

  const stats = resultats.statistics(nuancesPol);
  myLegend.categories(stats.nuances.list, {
    color: 'couleur',
    name: 'nom',
    shortName: 'code'
  }); //Injection des données de référence dans la boite de contenu

  contentBox.data(listeCircos, nuancesPol); //Création du selecteur (menu select)

  mySelector.data(listeCircos, [{
    value: 'REG_ID',
    text: 'REG_NOM',
    label: 'Région',
    placeholder: 'Région ou territoire',
    root: 'FRANCE ENTIERE',
    valueMapper: d => parseInt(d)
  }, {
    value: 'DEP_ID',
    text: 'DEP_NOM',
    label: 'Département',
    placeholder: 'Département',
    valueMapper: d => d.toString().padStart(3, '0')
  }, {
    value: 'CIRCO_ID',
    text: 'CIRCO_NOM',
