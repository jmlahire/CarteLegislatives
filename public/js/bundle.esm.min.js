...
const d3$5 = Object.assign({}, d3Selection, d3Dispatch, d3Transition);
...
class Component extends Queue {
  ...
  constructor(id) {
    ...
    this._style = {};
    this._dispatch = d3$5.dispatch("user");
    this._container = undefined;
    this._transition = d3$5.transition().duration(750);
    Registry.add(this);
  }
  /**
  ...
  appendTo(parent) {
    ...
    if (!this.outerContainer) console.warn(`Aucun contenu présent dans le composant ${this.id}`); //Determination du type de parent
      ...
      else if (parent instanceof d3$5.selection) {
        this._parent.container = parent;
      } //Selection d3 (passé par son id)
      else if (typeof parent === 'string') {
        this._parent.container = d3$5.select(`#${parent.replace('#', '')}`);
      } else this._parent.container = d3$5.select('body'); //Insertion dans le DOM
...
const d3$4 = Object.assign({}, d3Selection);
...
class Svg extends Component {
  ...
  constructor(id, width = 2000, height = 1200, margins = {
    ...
    this._outerContainer = d3$4.create('svg:svg').attr('id', this.id).attr('class', this.constructor.name).attr(`preserveAspectRatio`, 'xMaxYMin meet').attr('viewBox', `0 0 ${this.size.width} ${this.size.height}`).attr('width', `100%`);
...
class MapLayer extends Component {
  ...
  constructor(id, parent, autofit = true) {
    super(id);
    this._dispatch = d3$3.dispatch('user');
    this._container = d3$3.create('svg:g').attr('id', this.id).classed(this.type, true);
  ...
  get projection() {
    return this.parent.projection || d3$3.geoMercator();
  }
  /**
  ...
  get path() {
    return this.parent.path || d3$3.geoPath();
  }

  test() {
    this._dispatch.call('user', this, {
      layer: this.id,
      msg: 'Click'
    });
  }
...
const d3$2 = Object.assign({}, d3Selection, d3Fetch, d3Array, d3Geo, d3Zoom, d3Transition, d3Ease, d3Dispatch);
...
const d3$1 = Object.assign({}, d3Selection, d3Geo, d3Zoom, d3Transition, d3Ease, d3Dispatch);
...
class MapComposition extends Svg {
  ...
  constructor(id, size = {}, options = {}) {
    ...
    this.layers = new Map(); //this.defs = this.outerContainer.append('defs').lower();

    this._dispatch = d3$1.dispatch('zoom');
    this._zoomEvent = {
      e: null,
      level: 1,
      source: null
    };
    this._zoom = d3$1.zoom().scaleExtent([1, 15]).translateExtent([[0, 0], [this.size.width, this.size.height]]).on('zoom', e => this._handleZoom.call(this, e));
    this.outerContainer.call(this._zoom);
    this.freezoom(this.options.freezoom);
    this.projection = this.options.projection;
    this.path = d3$1.geoPath();
  }
  ...
  zoomTo(selection, zoomMargin = 1) {
    this._animationPending = new Promise((resolve, reject) => {
      ...
      const finalTransform = d3$1.zoomIdentity.translate(offset.x, offset.y).scale(scale);
      this.outerContainer.transition().delay(this.options.delay).duration(this.options.duration).call(this._zoom.transform, finalTransform).on('start', () => {
  ...
  zoomOut() {
    this._animationPending = new Promise((resolve, reject) => {
      this._freezoom = false;
      let finalTransform = d3$1.zoomIdentity.translate(0, 0).scale(1);
      this.outerContainer.transition().delay(this.options.delay).duration(this.options.duration).call(this._zoom.transform, finalTransform).on('start', () => {
        this.container.selectAll('g#departements path.area').style('pointer-events', 'none'); //Hack degueu
  ...
  fadeOutLayers(selector) {
    ...
    this.container.selectAll(`g${selector}`).transition().duration(this.options.duration / 2).style('opacity', 0).on('end', (d, i, n) => {
      d3$1.select(n[i]).style('display', 'none'); //   resolve(this);
    }); //    }));
...
d.appendTo(null).addLayer('circos').on('user.click', function (d) {
  console.log(d, this);
}).load('./../assets/geodata/circonscriptions.topojson'); //console.warn(d.getLayer('circos'));
