...
const d3$a = Object.assign({}, d3Selection, d3Dispatch, d3Transition);
...
class Component extends Queue {
  ...
  constructor(id) {
    ...
    this._style = {};
    this._dispatch = d3$a.dispatch('user');
    this._container = undefined;
    this._transition = d3$a.transition().duration(750);
    Registry.add(this);
  }
  /**
  ...
  appendTo(parent) {
    ...
    if (!this.outerContainer) console.warn(`Aucun contenu présent dans le composant ${this.id}`); //Determination du type de parent
      ...
      else if (parent instanceof d3$a.selection) {
        this._parent.container = parent;
      } //Selection d3 (passé par son id)
      else if (typeof parent === 'string') {
        this._parent.container = d3$a.select(`#${parent.replace('#', '')}`);
      } else this._parent.container = d3$a.select('body'); //Insertion dans le DOM
  ...
  test() {
    this.outerContainer.style('width', '100%').style('display', 'block').style('opacity', 0).transition(this._outerTransition).style('opacity', 1).transition(this._outerTransition).style('outline-color', 'blue').style('width', '50%');
  }
...
class Svg extends Component {
  ...
  constructor(id, width = 2000, height = 1200, margins = {
    ...
    this._outerContainer = d3$9.create('svg:svg').attr('id', this.id).attr(`preserveAspectRatio`, 'xMaxYMin meet').attr('viewBox', `0 0 ${this.size.width} ${this.size.height}`).attr('width', `100%`);
...
const d3$8 = Object.assign({}, d3Selection, d3Geo, d3Zoom, d3Transition, d3Ease, d3Dispatch);
...
class MapLayer extends Component {
  ...
  constructor(id, className = '', parent) {
    super(id);
    this._dispatch = d3$8.dispatch('click');
    this._container = d3$8.create('svg:g').attr('id', this.id).classed(className, className);
  ...
  get projection() {
    return this.parent.projection || d3$8.geoMercator();
  }
  /**
  ...
  get path() {
    return this.parent.path || d3$8.geoPath();
  }
...
const d3$7 = Object.assign({}, d3Selection, d3Fetch, d3Array, d3Geo, d3Zoom, d3Transition, d3Ease, d3Dispatch);
...
class MapPath extends MapLayer {
  ...
  load(arg) {
    ...
    this.parent.enqueue(() => new Promise((resolve, reject) => {
      d3$7.json(source).then(topology => {
  ...
  render() {
    ...
    this.parent.enqueue(() => new Promise((resolve, reject) => {
      ...
      this._container.selectAll("path").data(this.geodata, d => d.properties[this._options.primary]).join(enter => enter.append('path').attr('class', d => `path _${d.properties[this._options.primary]}`).classed('clickable', this._options.clickable).attr('d', this.path).on('click', (e, d) => {
        if (this._options.clickable) this._dispatch.call('click', this, {
          event: e,
          values: d.properties,
  ...
  statistics(keys, types = ['domain', 'mean']) {
    ...
    const calcFunction = {
      ...
      sum: d3$7.sum,
      count: d3$7.count,
      mean: v => calcRound(d3$7.mean(v)),
      median: v => calcRound(d3$7.median(v)),
      deviation: v => calcRound(d3$7.deviation(v, .1))
    };
    this.parent.enqueue(() => new Promise((resolve, reject) => {
    ...
    this.parent.enqueue(() => new Promise((resolve, reject) => {
      this._container.selectAll("path").each((d, i, n) => {
        const style = stylingFunction(d.properties),
              transition = d3$7.select(n[i]).transition().duration(this.parent._options.duration / 2); //Couleur simple
        ...
        if (typeof style.fill === 'string') {
          ...
          transition.style('fill', color).on('end', function (d) {
            d3$7.select(this).style('fill', `url(#${id})`);
          });
        } //Styles additionnels
  ...
  zoomTo(key, value, options = {}) {
    ...
    let selection = this.innerContainer.selectAll('path.clickable').classed('hidden', d => d.properties[key] !== value) //    .each(d=>console.log(d.properties[key],value))
    .filter(d => d.properties[key] === value); // .each(d=>console.log(d));
...
const d3$6 = Object.assign({}, d3Selection, d3Geo, d3Zoom, d3Transition, d3Ease, d3Dispatch);
...
class MapComposition extends Svg {
  ...
  constructor(id, size = {}, options = {}) {
    ...
    this._dispatch = d3$6.dispatch('zoom'); //this._zoomEvent={e:null,level:1,source:null};

    this._zoomSource = this.outerContainer;
    this._zoom = d3$6.zoom().scaleExtent([1, 15]).translateExtent([[0, 0], [this.size.width, this.size.height]]).on('zoom', e => this._handleZoom.call(this, e));

    this._zoomSource.call(this._zoom); // this.freezoom(this._options.freezoom);


    this.projection = this._options.projection;
    this.path = d3$6.geoPath();
  }
  /**
  ...
  zoomTo(selection, options = {}) {
    ...
    this.enqueue(() => new Promise((resolve, reject) => {
      ...
      const finalTransform = d3$6.zoomIdentity.translate(offset.x, offset.y).scale(scale);

      this._zoomSource.transition().delay(this._options.delay).duration(this._options.duration).call(this._zoom.transform, finalTransform).on('start', () => {
        console.log(this); //this.container.selectAll('g._mPth path').style('pointer-events','none'); //Hack degueu
        //   this.freezoom(false);
      }).on('end', () => {
        //                    const newBounds = getBoundaries(selection);
        this._zoom.scaleExtent([1, finalTransform.k * 4]); //   this.container.selectAll('g._mPth path').style('pointer-events','visible'); //Hack degueu


        this.outerContainer.call(this._zoom, finalTransform);
        console.log(finalTransform); // this.freezoom (this._options.freezoom);
  ...
  zoomOut() {
    this.enqueue(() => new Promise((resolve, reject) => {
      // this._freezoom = false;
      let finalTransform = d3$6.zoomIdentity.translate(0, 0).scale(1);
      this.outerContainer.transition().delay(this._options.delay).duration(this._options.duration).call(this._zoom.transform, finalTransform).on('start', () => {//       this.container.selectAll('g#departements path.area').style('pointer-events','none'); //Hack degueu
        // this.freezoom(false);
      }).on('end', () => {
  ...
  fadeOutLayers(selector) {
    ...
    this.container.selectAll(`g${selector}`).transition().duration(this._options.duration / 2).style('opacity', 0).on('end', (d, i, n) => {
      d3$6.select(n[i]).style('display', 'none'); //   resolve(this);
    }); //    }));
...
_defineProperty(MapComposition, "defaultOptions", {
  duration: 2000,
  delay: 0,
  projection: d3$6.geoMercator(),
  freezoom: false
});

const d3$5 = Object.assign({}, d3Selection, d3Dispatch);
...
class MapLegend extends Component {
  ...
  constructor(id, title = 'Légende') {
    super(id);
    this._outerContainer = d3$5.create('div').classed(MapLegend._type, true);
...
const d3$4 = Object.assign({}, d3Array, d3Fetch, d3Dsv);
...
class DataCollection {
  ...
  load(file, options = {}) {
    this.ready = new Promise((resolve, reject) => {
      ...
      d3$4.text(`${file}`).then(dataset => {
        dataset = d3$4.dsvFormat(options.delimiter).parse(dataset);
        if (options.dtype) dataset = applyDtype(dataset, options.dtype);else dataset = dataset.map(d3$4.autoType);
  ...
  toGroups(keys, output = 'array') {
    const groupFn = output === 'array' ? d3$4.groups : d3$4.group;
    if (typeof keys === 'string') keys = [keys];
  ...
  extent(key) {
    if (typeof key == 'string') {
      return d3$4.extent(this.col(key));
  ...
  sort(key, type = 'descending', inPlace = true) {
    let dataset = this.dataset.sort((a, b) => d3$4[type](a[key], b[key]));
...
class DropdownList {
  constructor(level, metadata, autoselect = true) {
    ...
    this._outerContainer = d3$3.create('li').attr('class', `ddList${level}`);
    this._innerContainer = this._outerContainer.append('select');
    this._dispatch = d3$3.dispatch('change');
  }
  ...
  findValue(value) {
    ...
    return found >= 0 ? d3$3.select(options[found]) : null;
  }
...
class NestedSelection extends Component {
  constructor(id) {
    ...
    this._outerContainer = d3$3.create('div').attr('id', this.id).classed(NestedSelection._type, true);
...
const d3$2 = Object.assign({}, d3Selection, d3Dispatch);
...
class FigureFactory {
  ...
  render(type, colors) {
    ...
    return d3$2.create('svg:svg').attr('preserveAspectRatio', 'xMaxYMin meet').attr('viewBox', `0 0 ${size.width} ${size.height}`).html(code);
  }
...
class Synthese extends Component {
  ...
  constructor(id) {
    super(id);
    this._container = d3$2.create('div').attr('id', id);
  ...
  _renderElus(data) {
    this._sectionElus.select('ul').selectAll('li').data(data).enter().append('li').each(function (d) {
      let circo = d[0],
          color = d[1].nuanceCol,
          figure = d[1].civ === 'M' ? 'man' : 'woman';
      let svg = new FigureFactory().render(figure, color);
      d3$2.select(this).attr('class', `_${circo}`).append(() => svg.node()).on('click', d => console.log(circo));
    });
  }

  _renderTriangulaires(data) {
    this._sectionTriang.select('ul').selectAll('li').data(data).enter().append('li').each(function (d) {
      let circo = d[0],
...
class Panel extends Component {
  ...
  resize() {
    ...
    this._handle.append('span').style('line-height', `${this.size.height}px`).style('font-size', `${this.size.handle * .8}px`).text('◀').classed('txt', () => this._options.handleText).filter(() => this._options.handleText).text(this._options.handleText).style('transform', `translateX(-${this.size.handle}px) rotate(-90deg)`); //Dimensionnement et positionnement contenu
  ...
  try {
    ...
    if (first) {
      ...
      if (first.pct_exprimes > 50 && first.pct_inscrits > 25) {
        ...
        colorA.l *= .5;
        colorB.l *= 1.2;
        fill = d3.create('svg:pattern').attr('id', `pat${first.nuanceId}`).attr('patternUnits', 'userSpaceOnUse').attr('width', 10).attr('height', 10).attr('patternTransform', 'rotate(45)');
        fill.append('rect').attr('x', 0).attr('y', 0).attr('width', 10).attr('height', 10).attr('fill', colorB.formatHex());
        fill.append('line').attr('x1', 0).attr('y1', 0).attr('x2', 0).attr('y2', 10).attr('stroke', color).attr('stroke-width', 5);
      } //Ballotage
      else {
        fill = color;
      }
...
Promise.all([nuancesPol.ready, resultats.ready, listeCircos.ready]).then(() => {
  myMap.layer('circos').load().fit().render().on('click', d => console.log(d)).join(resultats, "idcirco").fill(mapFillingFunction);
  ...
  console.log(urlParam); //Ajout de la légende
