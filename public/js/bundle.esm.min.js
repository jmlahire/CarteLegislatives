...
function selectAll$1 (selector) {
  return typeof selector === "string" ? new Selection$4([document.querySelectorAll(selector)], [document.documentElement]) : new Selection$4([array$3(selector)], root$3);
}
...
function bin() {
  ...
  function histogram(data) {
    ...
    if (!Array.isArray(tz)) {
      ...
      tz = ticks$1(x0, x1, tn); // If the domain is aligned with the first tick (which it will by
      // default), then we can use quantization rather than bisection to bin
      // values, which is substantially faster.

      if (tz[0] <= x0) step = tickIncrement$1(x0, x1, tn); // If the last threshold is coincident with the domain’s upper bound, the
...
function rank(values, valueof = ascending$3) {
  ...
  let k, r;
  Uint32Array.from(V, (_, i) => i).sort(valueof === ascending$3 ? (i, j) => ascendingDefined(V[i], V[j]) : compareDefined(compareIndex)).forEach((j, i) => {
...
function leastIndex$1(values, compare = ascending$3) {
...
function powish(transform) {
  ...
  function rescale() {
    return exponent === 1 ? transform(identity$a, identity$a) : exponent === 0.5 ? transform(transformSqrt, transformSquare) : transform(transformPow(exponent), transformPow(1 / exponent));
  }
...
function create$2(node, id, self) {
  ...
  function start(elapsed) {
    ...
    for (i in schedules) {
      ...
      if (o.state === STARTED$1) return timeout$2(start); // Interrupt the active transition, if any.
...
function transition_attr$1 (name, value) {
  var fullname = namespace$2(name),
      i = fullname === "transform" ? interpolateTransformSvg$1 : interpolate$3;
...
const d3$9 = Object.assign({}, d3Selection, d3Dispatch, d3Transition);
...
class Component extends Queue {
  ...
  constructor(id) {
    ...
    this._style = {};
    this._dispatch = d3$9.dispatch('user');
    this._container = undefined;
    this._transition = d3$9.transition().duration(750);
    Registry.add(this);
  }
  /**
  ...
  appendTo(parent) {
    ...
    if (!this.outerContainer) console.warn(`Aucun contenu présent dans le composant ${this.id}`); //Determination du type de parent
      ...
      else if (parent instanceof d3$9.selection) {
        this._parent.container = parent;
      } //Selection d3 (passé par son id)
      else if (typeof parent === 'string') {
        this._parent.container = d3$9.select(`#${parent.replace('#', '')}`);
      } else this._parent.container = d3$9.select('body'); //Insertion dans le DOM
...
class Svg extends Component {
  ...
  constructor(id, width = 2000, height = 1200, margins = {
    ...
    this._outerContainer = d3$8.create('svg:svg').attr('id', this.id).attr('class', this.constructor.name).attr(`preserveAspectRatio`, 'xMaxYMin meet').attr('viewBox', `0 0 ${this.size.width} ${this.size.height}`).attr('width', `100%`);
...
function zoom () {
  ...
  function wheeled(event, ...args) {
    ...
    var g = gesture(this, args).event(event),
        ...
        p = pointer$2(event); // If the mouse is in the same location as before, reuse it.
  ...
  function mousedowned(event, ...args) {
    ...
    var currentTarget = event.currentTarget,
        ...
        p = pointer$2(event, currentTarget),
        x0 = event.clientX,
        y0 = event.clientY;
    dragDisable(event.view);
    nopropagation(event);
    ...
    function mousemoved(event) {
      ...
      g.event(event).zoom("mouse", constrain(translate(g.that.__zoom, g.mouse[0] = pointer$2(event, currentTarget), g.mouse[1]), g.extent, translateExtent));
    }
  ...
  function dblclicked(event, ...args) {
    if (!filter.apply(this, arguments)) return;
    var t0 = this.__zoom,
        p0 = pointer$2(event.changedTouches ? event.changedTouches[0] : event, this),
        p1 = t0.invert(p0),
        k1 = t0.k * (event.shiftKey ? 0.5 : 2),
        t1 = constrain(translate(scale(t0, k1), p0, p1), extent.apply(this, args), translateExtent);
    noevent(event);
    if (duration > 0) select$2(this).transition().duration(duration).call(schedule, t1, p0, event);else select$2(this).call(zoom.transform, t1, p0, event);
  }
...
class MapLayer extends Component {
  ...
  constructor(id, parent) {
    super(id);
    this._dispatch = d3$7.dispatch('click');
    this._container = d3$7.create('svg:g').attr('id', this.id).classed(this.type, true);
  ...
  get projection() {
    return this.parent.projection || d3$7.geoMercator();
  }
  /**
  ...
  get path() {
    return this.parent.path || d3$7.geoPath();
  }
...
const d3$6 = Object.assign({}, d3Selection, d3Fetch, d3Array, d3Geo, d3Zoom, d3Transition, d3Ease, d3Dispatch);
...
class MapPath extends MapLayer {
  ...
  load(arg) {
    ...
    this.parent.enqueue(() => new Promise((resolve, reject) => {
      d3$6.json(source).then(topology => {
  ...
  render() {
    ...
    this.parent.enqueue(() => new Promise((resolve, reject) => {
      ...
      this._container.selectAll("path").data(this.geodata, d => d.properties[this._options.primary]).join(enter => enter.append('path').attr('class', d => `path _${d.properties[this._options.primary]}`).classed('clickable', this._options.clickable).attr('d', this.path).on('click', (e, d) => this._options.clickable ? this._dispatch.call('click', this, {
        event: e,
        values: d.properties,
        id: d.properties[this._options.primary]
      }) : null), update => update.call(update => update.transition().duration(0).attr('d', this.path)), exit => exit.remove());
  ...
  statistics(keys, types = ['domain', 'mean']) {
    ...
    const calcFunction = {
      ...
      sum: d3$6.sum,
      count: d3$6.count,
      mean: v => calcRound(d3$6.mean(v)),
      median: v => calcRound(d3$6.median(v)),
      deviation: v => calcRound(d3$6.deviation(v, .1))
    };
    this.parent.enqueue(() => new Promise((resolve, reject) => {
    ...
    this.parent.enqueue(() => new Promise((resolve, reject) => {
      this._container.selectAll("path").each((d, i, n) => {
        const style = stylingFunction(d.properties),
              transition = d3$6.select(n[i]).transition().duration(this.parent._options.duration / 2); //Couleur simple
        ...
        if (typeof style.fill === 'string') {
          ...
          transition.style('fill', color).on('end', function (d) {
            d3$6.select(this).style('fill', `url(#${id})`);
          });
        } //Styles additionnels
...
const d3$5 = Object.assign({}, d3Selection, d3Geo, d3Zoom, d3Transition, d3Ease, d3Dispatch);
...
class MapComposition extends Svg {
  ...
  constructor(id, size = {}, options = {}) {
    ...
    this._dispatch = d3$5.dispatch('zoom');
    this._zoomEvent = {
      e: null,
      level: 1,
      source: null
    };
    this._zoom = d3$5.zoom().scaleExtent([1, 15]).translateExtent([[0, 0], [this.size.width, this.size.height]]).on('zoom', e => this._handleZoom.call(this, e));
  ...
  zoomTo(selection, zoomMargin = 1) {
    this._animationPending = new Promise((resolve, reject) => {
      ...
      const finalTransform = d3$5.zoomIdentity.translate(offset.x, offset.y).scale(scale);
  ...
  zoomOut() {
    this._animationPending = new Promise((resolve, reject) => {
      this._freezoom = false;
      let finalTransform = d3$5.zoomIdentity.translate(0, 0).scale(1);
      this.outerContainer.transition().delay(this._options.delay).duration(this._options.duration).call(this._zoom.transform, finalTransform).on('start', () => {
        this.container.selectAll('g#departements path.area').style('pointer-events', 'none'); //Hack degueu
  ...
  fadeOutLayers(selector) {
    ...
    this.container.selectAll(`g${selector}`).transition().duration(this._options.duration / 2).style('opacity', 0).on('end', (d, i, n) => {
      d3$5.select(n[i]).style('display', 'none'); //   resolve(this);
    }); //    }));
...
class MapLegend extends Component {
  ...
  constructor(id) {
    super(id);
    this._outerContainer = d3$4.create('div').classed('_mapLegend', true);
  ...
  categories(data, meta = {}) {
    ...
    this._innerContainer.selectAll("li").data(data, d => d.id).join(enter => enter.append("li").each(function (d) {
      const elt = d3$4.select(this);
...
class DataCollection {
  ...
  load(file, options = {}) {
    this.ready = new Promise((resolve, reject) => {
      ...
      d3$3.text(`${file}`).then(dataset => {
        dataset = d3$3.dsvFormat(options.delimiter).parse(dataset);
        if (options.dtype) dataset = applyDtype(dataset, options.dtype);else dataset = dataset.map(d3$3.autoType);
  ...
  toGroups(keys, output = 'array') {
    const groupFn = output === 'array' ? d3$3.groups : d3$3.group;
    if (typeof keys === 'string') keys = [keys];
...
class DropdownList {
  constructor(level = null) {
    this.level = level;
    this._outerContainer = d3$2.create('li').classed(`select${level}`, level);
    this._innerContainer = this._outerContainer.append('select');
    this._dispatch = d3$2.dispatch('change');
  }
  ...
  update(data = null, structure = null) {
    ...
    if (data !== null) {
      ...
      if (data instanceof Map) {
        ...
        if (data.length === 1 && autoSelect) ; //this._dispatch.call('change',this,{level:this.level, value:'099' } );
        //this.hide();
...
class NestedSelection extends Component {
  constructor(id) {
    ...
    this._outerContainer = d3$2.create('div').attr('id', this.id).classed('_nestedSelection', true);
    this._innerContainer = this._outerContainer.append('ul');
  }
  /**
   *
...
const d3$1 = Object.assign({}, d3Selection, d3Dispatch);
...
class FigureFactory {
  ...
  render(type, colors) {
    ...
    return d3$1.create('svg:svg').attr('preserveAspectRatio', 'xMaxYMin meet').attr('viewBox', `0 0 ${size.width} ${size.height}`).html(code);
  }
...
class Synthese extends Component {
  ...
  constructor(id) {
    super(id);
    this._container = d3$1.create('div').attr('id', id);
  ...
  _renderElus(data) {
    this._sectionElus.select('ul').selectAll('li').data(data).enter().append('li').each(function (d) {
      ...
      d3$1.select(this).attr('class', `_${circo}`).append(() => svg.node()).on('click', d => console.log(circo));
    });
  }
