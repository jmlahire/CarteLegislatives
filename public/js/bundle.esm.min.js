...
function create$3(name) {
  return select$2(creator$1(name).call(document.documentElement));
}
...
function dragDisable(view) {
  var root = view.document.documentElement,
      selection = select$2(view).on("dragstart.drag", noevent$1, nonpassivecapture);
  if ("onselectstart" in root) {
...
class MapComposition extends Svg {
  ...
  constructor(id, size = {}, options = {}) {
    ...
    this._zoom = d3$6.zoom().filter(e => !e.button && e.type != "dblclick").scaleExtent([1, 15]).translateExtent([[0, 0], [this.size.width, this.size.height]]).on('zoom', e => this._handleZoom.call(this, e));
...
class Synthese extends Component {
  ...
  constructor(id) {
    ...
    this._dispatch = d3$2.dispatch('select');
  ...
  _renderElus(data) {
    if (!data.size) this._sectionElus.style('display', 'none');else this._sectionElus.select('ul').selectAll('li').data(data).enter().append('li').each((d, i, n) => {
      ...
      d3$2.select(n[i]).attr('class', `_${circo}`).append(() => svg.node()).on('click', () => this._dispatch.call('select', this, {
        circo: circo
      }));
    });
  }
...
Promise.all([nuancesPol.ready, resultats.ready, listeCircos.ready]).then(() => {
  myMap.layer('circos').load().fit().render().on('click', d => select([null, null, d.id], d.event)).join(resultats, "idcirco").fill(mapFillingFunction);
