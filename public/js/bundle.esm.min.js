...
function selection_style$2 (name, value, priority) {
  return arguments.length > 1 ? this.each((value == null ? styleRemove$4 : typeof value === "function" ? styleFunction$5 : styleConstant$4)(name, value, priority == null ? "" : priority)) : styleValue$2(this.node(), name);
}
...
function selection_style$1 (name, value, priority) {
  return arguments.length > 1 ? this.each((value == null ? styleRemove$2 : typeof value === "function" ? styleFunction$3 : styleConstant$2)(name, value, priority == null ? "" : priority)) : styleValue$1(this.node(), name);
}
...
function selection_style (name, value, priority) {
  return arguments.length > 1 ? this.each((value == null ? styleRemove : typeof value === "function" ? styleFunction$1 : styleConstant)(name, value, priority == null ? "" : priority)) : styleValue(this.node(), name);
}
...
const d3$7 = Object.assign({}, d3Selection, d3Dispatch, d3Transition);
...
class Component extends Queue {
  ...
  constructor(id) {
    ...
    this._style = {};
    this._dispatch = d3$7.dispatch('user');
    this._container = undefined;
    this._transition = d3$7.transition().duration(750);
    Registry.add(this);
  }
  /**
  ...
  appendTo(parent) {
    ...
    if (!this.outerContainer) console.warn(`Aucun contenu présent dans le composant ${this.id}`); //Determination du type de parent
      ...
      else if (parent instanceof d3$7.selection) {
        this._parent.container = parent;
      } //Selection d3 (passé par son id)
      else if (typeof parent === 'string') {
        this._parent.container = d3$7.select(`#${parent.replace('#', '')}`);
      } else this._parent.container = d3$7.select('body'); //Insertion dans le DOM
...
class Svg extends Component {
  ...
  constructor(id, width = 2000, height = 1200, margins = {
    ...
    this._outerContainer = d3$6.create('svg:svg').attr('id', this.id).attr('class', this.constructor.name).attr(`preserveAspectRatio`, 'xMaxYMin meet').attr('viewBox', `0 0 ${this.size.width} ${this.size.height}`).attr('width', `100%`);
...
class MapLayer extends Component {
  ...
  constructor(id, parent) {
    super(id);
    this._dispatch = d3$5.dispatch('click');
    this._container = d3$5.create('svg:g').attr('id', this.id).classed(this.type, true);
  ...
  get projection() {
    return this.parent.projection || d3$5.geoMercator();
  }
  /**
  ...
  get path() {
    return this.parent.path || d3$5.geoPath();
  }
...
const d3$4 = Object.assign({}, d3Selection, d3Fetch, d3Array, d3Geo, d3Zoom, d3Transition, d3Ease, d3Dispatch);
...
class MapPath extends MapLayer {
  ...
  load(arg) {
    ...
    this.parent.enqueue(() => new Promise((resolve, reject) => {
      d3$4.json(source).then(topology => {
  ...
  statistics(keys, types = ['domain', 'mean']) {
    ...
    const calcFunction = {
      ...
      sum: d3$4.sum,
      count: d3$4.count,
      mean: v => calcRound(d3$4.mean(v)),
      median: v => calcRound(d3$4.median(v)),
      deviation: v => calcRound(d3$4.deviation(v, .1))
    };
    this.parent.enqueue(() => new Promise((resolve, reject) => {
  ...
  fill(styleFunction) {
    this.parent.enqueue(() => new Promise((resolve, reject) => {
      this._container.selectAll("path").each((d, i, n) => {
        ...
        let transition = d3$4.select(n[i]).transition().duration(this.parent._options.duration / 2).on('end', () => resolve(this)); //Couleur simple
        ...
        if (typeof style.fill === 'string') {
          ...
          transition.style('fill', color).on('end', function (d) {
            let elt = d3$4.select(this); //console.log(d,this)
...
const d3$3 = Object.assign({}, d3Selection, d3Geo, d3Zoom, d3Transition, d3Ease, d3Dispatch);
...
class MapComposition extends Svg {
  ...
  constructor(id, size = {}, options = {}) {
    ...
    this._dispatch = d3$3.dispatch('zoom');
    this._zoomEvent = {
      e: null,
      level: 1,
      source: null
    };
    this._zoom = d3$3.zoom().scaleExtent([1, 15]).translateExtent([[0, 0], [this.size.width, this.size.height]]).on('zoom', e => this._handleZoom.call(this, e));
    this.outerContainer.call(this._zoom);
    this.freezoom(this._options.freezoom);
    this.projection = this._options.projection;
    this.path = d3$3.geoPath();
  }
  /**
  ...
  zoomTo(selection, zoomMargin = 1) {
    this._animationPending = new Promise((resolve, reject) => {
      ...
      const finalTransform = d3$3.zoomIdentity.translate(offset.x, offset.y).scale(scale);
  ...
  zoomOut() {
    this._animationPending = new Promise((resolve, reject) => {
      this._freezoom = false;
      let finalTransform = d3$3.zoomIdentity.translate(0, 0).scale(1);
      this.outerContainer.transition().delay(this._options.delay).duration(this._options.duration).call(this._zoom.transform, finalTransform).on('start', () => {
        this.container.selectAll('g#departements path.area').style('pointer-events', 'none'); //Hack degueu
  ...
  fadeOutLayers(selector) {
    ...
    this.container.selectAll(`g${selector}`).transition().duration(this._options.duration / 2).style('opacity', 0).on('end', (d, i, n) => {
      d3$3.select(n[i]).style('display', 'none'); //   resolve(this);
    }); //    }));
...
_defineProperty(MapComposition, "type", 'MapComposition');

_defineProperty(MapComposition, "defaultOptions", {
  duration: 2000,
  delay: 0,
  projection: d3$3.geoMercator(),
  freezoom: false
});

let _Symbol$iterator;
const d3$2 = Object.assign({}, d3Array, d3Fetch, d3Dsv);
...
class DataCollection {
  ...
  load(file, options = {}) {
    this.ready = new Promise((resolve, reject) => {
      ...
      d3$2.text(`${file}`).then(dataset => {
        dataset = d3$2.dsvFormat(options.delimiter).parse(dataset);
        if (options.dtype) dataset = applyDtype(dataset, options.dtype);else dataset = dataset.map(d3$2.autoType);
  ...
  toGroups(keys) {
    ...
    let fns = keys.map(k => d => d[k]),
        nested = d3$2.groups(this.dataset, ...fns); //A vérifier que ça marche avec plusieurs clés...
  ...
  extent(key) {
    if (typeof key == 'string') {
      return d3$2.extent(this.col(key));
  ...
  sort(key, type = 'descending', inPlace = true) {
    let dataset = this.dataset.sort((a, b) => d3$2[type](a[key], b[key]));
...
class NavSelect extends Component {
  constructor(id, options = {}) {
    super(id);
    this.options = { ...NavSelect.defaultOptions,
      ...options
    };
    this._dispatch = d3$1.dispatch('change');
    this._container = d3$1.create('div').attr('id', this.id).classed(NavSelect.type, true);
    this._form = this._container.append('form');

    this._form.append('label').attr('for', this.id).text(this.options.label);

    this.selectNode = this._form.append('select').attr('id', this.id).attr('name', this.id);
...
_defineProperty(NavSelect, "type", '_Select');

_defineProperty(NavSelect, "defaultOptions", {
  required: true,
  label: ''
});
...
const nuances = new DataCollection("nuances").load("./assets/data/nuances.csv", {
  primary: "id"
});
...
let listeCircos = new DataCollection('listeCircos').load("./assets/data/listecircos.csv", {
  delimiter: ';',
  primary: 'CIRCO_ID',
  dtype: {
    CIRCO_ID: "string"
...
function styleFunction(properties) {
  ...
  try {
    const data = properties.JDATA.sort((a, b) => d3.descending(a.voix, b.voix)),
          first = data[0],
          color = nuances.map.get(first.nuanceId)[0].couleur; //Election au premier tour
...
Promise.all([nuances.ready, resultats.ready, listeCircos.ready]).then(() => {
  myMap.layer('circos', {
    ...
    autofit: true,
    clickable: true
  }).load().fit().render().on('click', d => console.log(d)).join(resultats, "idcirco").fill(styleFunction);
  myMap.layer('depts', {
    type: 'path',
  ...
  new NavSelect('region').appendTo('header');
  let nestedCircos = [new Map(), new Map(), new Map()];
  listeCircos.forEach(r => {
    nestedCircos[0].set(r[0], {
      label: r[1][0][1][0]['REG_NOM']
    });
    r[1].forEach(d => {
      console.log(d);
      nestedCircos[1].set(d[0], {
        label: d[1][0]['DEP_NOM']
      });
    });
  });
