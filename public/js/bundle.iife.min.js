(function () {
  ...
  class MapPath extends MapLayer {
    ...
    highlight(id) {
      this._container.selectAll('path.clickable').classed('highlight', d => d.properties[this._options.primary] === id).filter(d => d.properties[this._options.primary] === id).raise();
    }
  ...
  class DropdownList {
    constructor(level = null, options = {}) {
      ...
      this._outerContainer = d3$3.create('li').classed(`select${level}`, level);
    ...
    update(data = null, structure = null) {
      ...
      if (data !== null) {
        this.show(); //Niveaux supérieurs (données dans une Map)

        if (data instanceof Map) {
          this._innerContainer.selectAll('option').data(data).enter().append('option').attr('value', d => d[0]).text(d => this._accessor(d[1]));
          ...
          this._innerContainer.selectAll('option').data(data).enter().append('option').attr('value', d => d[this._structure.value]).text(d => d[this._structure.text]);

          if (data.length === 1 && this._options.autoselect) ; //this._dispatch.call('change',this,{level:this.level, value:'099' } );
          //this.hide();
        ...
        this._innerContainer.on('change', e => {
          ...
          if (e.target.value === "###") Object.assign(msg, {
            value: null,
            root: true
          });
    ...
    addPlaceHolder(text) {
      this._innerContainer.append('option').property('disabled', true).property('selected', true).property('hidden', true).text(text);
    ...
    addOption(value, label, first = true) {
      if (this.findValue(value) === null) {
        let option = this._innerContainer.append('option').attr('value', value).text(label);
  ...
  _defineProperty(DropdownList, "_options", {
    autoselect: true
  });
  ...
  class NestedSelection extends Component {
    ...
    data(data, metadata, options = {}) {
      ...
      for (let i = 0; i <= size - 1; i++) {
        ...
        this._sections[i] = new DropdownList(i).appendTo(this).update(i === size - 1 ? dataset : null, structure[i]).on('change', msg => {
