(function () {
  ...
  const d3$6 = Object.assign({}, d3Selection, d3Dispatch, d3Transition);
  ...
  class Component extends Queue {
    ...
    constructor(id) {
      ...
      this._style = {};
      this._dispatch = d3$6.dispatch('user');
      this._container = undefined;
      this._transition = d3$6.transition().duration(750);
      Registry.add(this);
    }
    /**
    ...
    appendTo(parent) {
      ...
      if (!this.outerContainer) console.warn(`Aucun contenu présent dans le composant ${this.id}`); //Determination du type de parent
        ...
        else if (parent instanceof d3$6.selection) {
          this._parent.container = parent;
        } //Selection d3 (passé par son id)
        else if (typeof parent === 'string') {
          this._parent.container = d3$6.select(`#${parent.replace('#', '')}`);
        } else this._parent.container = d3$6.select('body'); //Insertion dans le DOM
  ...
  const d3$5 = Object.assign({}, d3Selection);
  ...
  class Svg extends Component {
    ...
    constructor(id, width = 2000, height = 1200, margins = {
      ...
      this._outerContainer = d3$5.create('svg:svg').attr('id', this.id).attr('class', this.constructor.name).attr(`preserveAspectRatio`, 'xMaxYMin meet').attr('viewBox', `0 0 ${this.size.width} ${this.size.height}`).attr('width', `100%`);
  ...
  const d3$4 = Object.assign({}, d3Selection, d3Geo, d3Zoom, d3Transition, d3Ease, d3Dispatch);
  ...
  class MapLayer extends Component {
    ...
    constructor(id, parent) {
      super(id);
      this._dispatch = d3$4.dispatch('click');
      this._container = d3$4.create('svg:g').attr('id', this.id).classed(this.type, true);
    ...
    get projection() {
      return this.parent.projection || d3$4.geoMercator();
    }
    /**
    ...
    get path() {
      return this.parent.path || d3$4.geoPath();
    }
  ...
  const d3$3 = Object.assign({}, d3Selection, d3Fetch, d3Array, d3Geo, d3Zoom, d3Transition, d3Ease, d3Dispatch);
  ...
  class MapPath extends MapLayer {
    ...
    load(arg) {
      ...
      this.parent.enqueue(() => new Promise((resolve, reject) => {
        d3$3.json(source).then(topology => {
    ...
    render() {
      ...
      this.parent.enqueue(() => new Promise((resolve, reject) => {
        ...
        this._container.selectAll("path").data(this.geodata).enter().append('path').attr('class', d => `path _${d.properties[this._options.primary]}`).classed('clickable', this._options.clickable).attr('d', this.path).on('click', (e, d) => this._options.clickable ? this._dispatch.call('click', this, {
          event: e,
          values: d.properties,
    ...
    statistics(keys, types = ['domain', 'mean']) {
      ...
      const calcFunction = {
        ...
        sum: d3$3.sum,
        count: d3$3.count,
        mean: v => calcRound(d3$3.mean(v)),
        median: v => calcRound(d3$3.median(v)),
        deviation: v => calcRound(d3$3.deviation(v, .1))
      };
      this.parent.enqueue(() => new Promise((resolve, reject) => {
    ...
    fill(styleFunction) {
      this.parent.enqueue(() => new Promise((resolve, reject) => {
        this._container.selectAll("path").each((d, i, n) => {
          ...
          if (style.fill) d3$3.select(n[i]).style('fill', style.fill);
          if (style.stroke) d3$3.select(n[i]).style('stroke', style.stroke);
          if (style.strokeWidth) d3$3.select(n[i]).style('stroke-width', style.strokeWidth);
        });
  ...
  class MapComposition extends Svg {
    ...
    constructor(id, size = {}, options = {}) {
      ...
      this._dispatch = d3$2.dispatch('zoom');
      this._zoomEvent = {
        e: null,
        level: 1,
        source: null
      };
      this._zoom = d3$2.zoom().scaleExtent([1, 15]).translateExtent([[0, 0], [this.size.width, this.size.height]]).on('zoom', e => this._handleZoom.call(this, e));
  ...
  _defineProperty(MapComposition, "defaultOptions", {
    duration: 2000,
    delay: 0,
    projection: d3$2.geoMercator(),
    freezoom: false
  });

  let _Symbol$iterator;
  const d3$1 = Object.assign({}, d3Array, d3Fetch, d3Dsv);
  ...
  class DataCollection {
    ...
    load(file, options = {}) {
      this.ready = new Promise((resolve, reject) => {
        ...
        d3$1.text(`${file}`).then(dataset => {
          dataset = d3$1.dsvFormat(options.delimiter).parse(dataset);
          if (options.dtype) dataset = applyDtype(dataset, options.dtype);else dataset = dataset.map(d3$1.autoType);
    ...
    toGroups(keys) {
      ...
      let fns = keys.map(k => d => d[k]),
          nested = d3$1.groups(this.dataset, ...fns); //A vérifier que ça marche avec plusieurs clés...
    ...
    extent(key) {
      if (typeof key == 'string') {
        return d3$1.extent(this.col(key));
    ...
    sort(key, type = 'descending', inPlace = true) {
      let dataset = this.dataset.sort((a, b) => d3$1[type](a[key], b[key]));
  ...
  nuances.load("./assets/data/nuances.csv", {
    primary: "id"
  });
  ...
  function styleFunction(properties) {
    ...
    try {
      ...
      fill = d3.hsl(fill); //Election au premier tour
  ...
  Promise.all([nuances.ready, resultats.ready]).then(() => {
    ...
    .render().on('click', d => console.log(d)).join(resultats, "idcirco").fill(styleFunction);
    myMap.layer('depts', {
      type: 'path',
